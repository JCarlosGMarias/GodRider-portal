            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    <li class="nav-item" v-for="(menu, index) in menuOptions">
                        ...
                        <a v-else-if="menu.name == 'Logout'" class="nav-link" aria-current="page" href="#" @click="logout()">{{ menu.name }}</a>
                        <a v-else class="nav-link" aria-current="page" href="#" @click="renderMenu(index)">{{ menu.name }}</a>
                    </li>
                </ul>
            </div>
        </div>
